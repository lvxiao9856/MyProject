...
import TextProcess.ChiSquaretest;
...
public class Model {	
	...
	public boolean saveModelTwords(String filename){
		try{
			...
			Map<Integer,List<String>> topic_words=new HashMap<Integer,List<String>>();
			for (int k = 0; k < K; k++){
				...
				topic_words.put(k, words);
			} //end foreach topic
			ChiSquaretest aa=new ChiSquaretest();
			aa.caculate_chi(topic_words);
			writer.close();
		}
		catch(Exception e){
